...
package tv.danmaku.cap.media.sample.widget.preference;
...
public class CapListPreference extends ListPreference {
    private CharSequence[] mEntrySummaries;

    public CapListPreference(Context context) {
        super(context);
        initPreference(context, null);
    }

    public CapListPreference(Context context, AttributeSet attrs) {
        super(context, attrs);
        initPreference(context, attrs);
    }

    public CapListPreference(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initPreference(context, attrs);
    }

    public CapListPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private final void initPreference(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.CapListPreference, 0, 0);
        if (a == null)
            return;

        mEntrySummaries = a
                .getTextArray(R.styleable.CapListPreference_entrySummaries);
