...
import tv.danmaku.cap.media.player.annotations.AccessedByNative;
import tv.danmaku.cap.media.player.annotations.CalledByNative;
import tv.danmaku.cap.media.player.misc.IMediaDataSource;
import tv.danmaku.cap.media.player.misc.ITrackInfo;
import tv.danmaku.cap.media.player.misc.CapTrackInfo;
import tv.danmaku.cap.media.player.pragma.DebugLog;
 ...
 */
public final class CapMediaPlayer extends AbstractMediaPlayer {
    private final static String TAG = CapMediaPlayer.class.getName();
    ...
    //----------------------------------------
    // options
    public static final int CAP_LOG_UNKNOWN = 0;
    public static final int CAP_LOG_DEFAULT = 1;

    public static final int CAP_LOG_VERBOSE = 2;
    public static final int CAP_LOG_DEBUG = 3;
    public static final int CAP_LOG_INFO = 4;
    public static final int CAP_LOG_WARN = 5;
    public static final int CAP_LOG_ERROR = 6;
    public static final int CAP_LOG_FATAL = 7;
    public static final int CAP_LOG_SILENT = 8;
     ...
     */
    private static CapLibLoader sLocalLibLoader = new CapLibLoader() {
        @Override
    ...
    public static void loadLibrariesOnce(CapLibLoader libLoader) {
        synchronized (CapMediaPlayer.class) {
            if (!mIsLibLoaded) {
                if (libLoader == null)
                    libLoader = sLocalLibLoader;

                libLoader.loadLibrary("capffmpeg");
                libLoader.loadLibrary("capsdl");
                libLoader.loadLibrary("caplayer");
                mIsLibLoaded = true;
            }
        }
    }
     ...
     * synchronously instantiating a CapMediaPlayer from a Uri or resource.
     * <p>
     * When done with the CapMediaPlayer, you should call {@link #release()}, to
     * free the resources. If not released, too many CapMediaPlayer instances
     * may result in an exception.
     * </p>
     */
    public CapMediaPlayer() {
     ...
     */
    public CapMediaPlayer(CapLibLoader libLoader) {
        initPlayer(libLoader);
    }

    private void initPlayer(CapLibLoader libLoader) {
         ...
         */
        native_setup(new WeakReference<CapMediaPlayer>(this));
    }

    /*
     * Update the CapMediaPlayer SurfaceTexture. Call after setting a new
     * display surface.
     */
    ...
    @Override
    public CapTrackInfo[] getTrackInfo() {
        Bundle bundle = getMediaMeta();
        if (bundle == null)
            return null;

        CapMediaMeta mediaMeta = CapMediaMeta.parse(bundle);
        if (mediaMeta == null || mediaMeta.mStreams == null)
            return null;

        ArrayList<CapTrackInfo> trackInfos = new ArrayList<CapTrackInfo>();
        for (CapMediaMeta.CapStreamMeta streamMeta: mediaMeta.mStreams) {
            CapTrackInfo trackInfo = new CapTrackInfo(streamMeta);
            if (streamMeta.mType.equalsIgnoreCase(CapMediaMeta.CAPM_VAL_TYPE__VIDEO)) {
                trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_VIDEO);
            } else if (streamMeta.mType.equalsIgnoreCase(CapMediaMeta.CAPM_VAL_TYPE__AUDIO)) {
                trackInfo.setTrackType(ITrackInfo.MEDIA_TRACK_TYPE_AUDIO);
            }
            trackInfos.add(trackInfo);
        }

        return trackInfos.toArray(new CapTrackInfo[trackInfos.size()]);
    }
    ...
    /**
     * Releases resources associated with this CapMediaPlayer object. It is
     * considered good practice to call this method when you're done using the
     * CapMediaPlayer. In particular, whenever an Activity of an application is
     * paused (its onPause() method is called), or stopped (its onStop() method
     * is called), this method should be invoked to release the CapMediaPlayer
     ...
     * if a CapMediaPlayer object is no longer needed may also lead to
    ...
    public MediaInfo getMediaInfo() {
        ...
        mediaInfo.mMediaPlayerName = "caplayer";
        ...
        try {
            mediaInfo.mMeta = CapMediaMeta.parse(_getMediaMeta());
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return mediaInfo;
    ...
    private native void native_setup(Object CapMediaPlayer_this);

    private native void native_finalize();

    private native void native_message_loop(Object CapMediaPlayer_this);
    ...
    private static class EventHandler extends Handler {
        private WeakReference<CapMediaPlayer> mWeakPlayer;

        public EventHandler(CapMediaPlayer mp, Looper looper) {
            super(looper);
            mWeakPlayer = new WeakReference<CapMediaPlayer>(mp);
        }

        @Override
        public void handleMessage(Message msg) {
            CapMediaPlayer player = mWeakPlayer.get();
            if (player == null || player.mNativeMediaPlayer == 0) {
                DebugLog.w(TAG,
                        "CapMediaPlayer went away with unhandled events");
                return;
            }
     ...
     * thread. We use a weak reference to the original CapMediaPlayer object so
    ...
    private static void postEventFromNative(Object weakThiz, int what,
        ...
        CapMediaPlayer mp = (CapMediaPlayer) ((WeakReference) weakThiz).get();
        if (mp == null) {
            return;
        }
    ...
    private static boolean onNativeInvoke(Object weakThiz, int what, Bundle args) {
        ...
        WeakReference<CapMediaPlayer> weakPlayer = (WeakReference<CapMediaPlayer>) weakThiz;
        CapMediaPlayer player = weakPlayer.get();
        if (player == null)
    ...
    public static class DefaultMediaCodecSelector implements OnMediaCodecSelectListener {
        ...
        public String onMediaCodecSelect(IMediaPlayer mp, String mimeType, int profile, int level) {
            ...
            ArrayList<CapMediaCodecInfo> candidateCodecList = new ArrayList<CapMediaCodecInfo>();
            ...
            for (int i = 0; i < numCodecs; i++) {
                ...
                for(String type: types) {
                    ...
                    CapMediaCodecInfo candidate = CapMediaCodecInfo.setupCandidate(codecInfo, mimeType);
                    if (candidate == null)
                        continue;
            ...
            CapMediaCodecInfo bestCodec = candidateCodecList.get(0);

            for (CapMediaCodecInfo codec : candidateCodecList) {
                if (codec.mRank > bestCodec.mRank) {
                    bestCodec = codec;
                }
            }

            if (bestCodec.mRank < CapMediaCodecInfo.RANK_LAST_CHANCE) {
