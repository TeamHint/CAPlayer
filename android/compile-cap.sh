...
do_ndk_build () {
    ...
    case "$PARAM_TARGET" in
        armv5|armv7a)
            cd "caplayer/caplayer-$PARAM_TARGET/src/main/jni"
...
case "$REQUEST_TARGET" in
    ...
    *)
        echo "Usage:"
        echo "  compile-cap.sh armv5|armv7a|x86|arm64"
        echo "  compile-cap.sh all|all32"
        echo "  compile-cap.sh all64"
        echo "  compile-cap.sh clean"
    ;;
esac
