...
#include "capsdl/android/capsdl_codec_android_mediaformat_java.h"
#include "capsdl/android/capsdl_codec_android_mediacodec_java.h"
#include "capsdl/android/capsdl_vout_android_nativewindow.h"
#include "capsdl/android/capsdl_vout_overlay_android_mediacodec.h"
#include "h264_nal.h"
#include "hevc_nal.h"
...
typedef struct CAPFF_Pipenode_Opaque {
    FFPlayer                 *ffp;
    CAPFF_Pipeline           *pipeline;
    Decoder                  *decoder;
    SDL_Vout                 *weak_vout;

    capmp_mediacodecinfo_context mcc;
...
} CAPFF_Pipenode_Opaque;

static SDL_AMediaCodec *create_codec_l(JNIEnv *env, CAPFF_Pipenode *node)
{
    CAPFF_Pipenode_Opaque        *opaque   = node->opaque;
    capmp_mediacodecinfo_context *mcc      = &opaque->mcc;
...
static int reconfigure_codec_l(JNIEnv *env, CAPFF_Pipenode *node, jobject new_surface)
{
    CAPFF_Pipenode_Opaque *opaque   = node->opaque;
    int                    ret      = 0;
...
#if 0
static int reconfigure_codec(JNIEnv *env, CAPFF_Pipenode *node)
{
    CAPFF_Pipenode_Opaque *opaque = node->opaque;
...
static int amc_fill_frame_fake(CAPFF_Pipenode *node, AVFrame *frame, int *got_frame, AVPacket *pkt)
{
    CAPFF_Pipenode_Opaque *opaque     = node->opaque;
    FFPlayer              *ffp        = opaque->ffp;
    VideoState            *is         = ffp->is;
...
static int amc_decode_picture_fake(CAPFF_Pipenode *node, uint32_t timeout_milli)
{
    CAPFF_Pipenode_Opaque *opaque   = node->opaque;
    FFPlayer              *ffp      = opaque->ffp;
    VideoState            *is       = ffp->is;
...
static int feed_input_buffer(JNIEnv *env, CAPFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    CAPFF_Pipenode_Opaque *opaque   = node->opaque;
    FFPlayer              *ffp      = opaque->ffp;
    CAPFF_Pipeline        *pipeline = opaque->pipeline;
    VideoState            *is       = ffp->is;
    Decoder               *d        = &is->viddec;
...
static double pts_from_buffer_info(CAPFF_Pipenode *node, SDL_AMediaCodecBufferInfo *buffer_info)
{
    CAPFF_Pipenode_Opaque *opaque     = node->opaque;
    FFPlayer              *ffp        = opaque->ffp;
    VideoState            *is         = ffp->is;
...
static int drain_output_buffer_l(JNIEnv *env, CAPFF_Pipenode *node, int64_t timeUs, int *dequeue_count, AVFrame *frame, int *got_frame)
{
    CAPFF_Pipenode_Opaque *opaque   = node->opaque;
    FFPlayer              *ffp      = opaque->ffp;
    int                    ret      = 0;
...
static int drain_output_buffer(JNIEnv *env, CAPFF_Pipenode *node, int64_t timeUs, int *dequeue_count, AVFrame *frame, int *got_frame)
{
    CAPFF_Pipenode_Opaque *opaque = node->opaque;
    SDL_LockMutex(opaque->acodec_mutex);
...
static int func_flush(CAPFF_Pipenode *node)
{
    CAPFF_Pipenode_Opaque *opaque   = node->opaque;
...
CAPFF_Pipenode *ffpipenode_create_video_decoder_from_android_mediacodec(FFPlayer *ffp, CAPFF_Pipeline *pipeline, SDL_Vout *vout)
{
    ...
    if (SDL_Android_GetApiLevel() < CAP_API_16_JELLY_BEAN)
        return NULL;
    ...
    if (ffp->mediacodec_auto_rotate &&
        rotate_degrees != 0 &&
        SDL_Android_GetApiLevel() >= CAP_API_21_LOLLIPOP) {
