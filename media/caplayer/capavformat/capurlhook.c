...
static int capurlhook_call_inject(URLContext *h)
{
    ...
    if (opaque && inject_callback) {
        CAPAVInject_OnUrlOpenData inject_data_backup = c->inject_data;
...
static int capurlhook_reconnect(URLContext *h, AVDictionary *extra)
{
    Context *c = h->priv_data;
    int ret = 0;
...
static int capurlhook_init(URLContext *h, const char *arg, int flags, AVDictionary **options)
{
    Context *c = h->priv_data;
    int ret = 0;
    ...
    av_dict_set_int(&c->inner_options, "capinject-opaque",        c->opaque, 0);
    av_dict_set_int(&c->inner_options, "capinject-segment-index", c->segment_index, 0);
...
static int captcphook_open(URLContext *h, const char *arg, int flags, AVDictionary **options)
{
    Context *c = h->priv_data;
    int ret = 0;
    ...
    c->scheme = "captcphook:";
    c->inner_scheme = "tcp:";
    c->open_callback_id = CAPAVINJECT_ON_TCP_OPEN;
    ret = capurlhook_init(h, arg, flags, options);
    if (ret)
        goto fail;

    ret = capurlhook_call_inject(h);
    if (ret)
        goto fail;

    ret = capurlhook_reconnect(h, NULL);
    if (ret)
        goto fail;
...
static int capurlhook_close(URLContext *h)
{
    Context *c = h->priv_data;
...
static int capurlhook_read(URLContext *h, unsigned char *buf, int size)
{
    Context *c = h->priv_data;
    int ret = 0;
...
static int capurlhook_write(URLContext *h, const unsigned char *buf, int size)
{
    Context *c = h->priv_data;
...
static int64_t capurlhook_seek(URLContext *h, int64_t pos, int whence)
{
    Context *c = h->priv_data;
    int64_t seek_ret = 0;
...
static int caphttphook_reconnect_at(URLContext *h, int64_t offset)
{
    AVDictionary *extra_opts = NULL;

    av_dict_set_int(&extra_opts, "offset", offset, 0);
    int ret = capurlhook_reconnect(h, extra_opts);
    av_dict_free(&extra_opts);
    return ret;
}

static int caphttphook_open(URLContext *h, const char *arg, int flags, AVDictionary **options)
{
    Context *c = h->priv_data;
    int ret = 0;

    c->scheme = "caphttphook:";
    c->inner_scheme = "http:";
    c->open_callback_id = CAPAVINJECT_ON_HTTP_OPEN;
...
static int caphttphook_read(URLContext *h, unsigned char *buf, int size)
{
    Context *c = h->priv_data;
    int ret = 0;
    ...
    read_ret = capurlhook_read(h, buf, size);
    while (read_ret < 0 && !h->is_streamed && c->logical_pos < c->logical_size) {
        ...
        ret = caphttphook_reconnect_at(h, c->logical_pos);
...
static int64_t caphttphook_reseek_at(URLContext *h, int64_t pos, int whence, int force_reconnect)
{
    Context *c = h->priv_data;
    int ret = 0;

    if (!force_reconnect)
        return capurlhook_seek(h, pos, whence);
    ...
    ret = caphttphook_reconnect_at(h, pos);
    if (ret) {
        c->io_error = ret;
        return ret;
    }
...
static int64_t caphttphook_seek(URLContext *h, int64_t pos, int whence)
{
    Context *c = h->priv_data;
    int     ret      = 0;
    int64_t seek_ret = -1;
    ...
    seek_ret = caphttphook_reseek_at(h, pos, whence, c->inject_data.is_url_changed);
    while (seek_ret < 0) {
        switch (seek_ret) {
            case AVERROR_EXIT:
            case AVERROR_EOF:
...
static const AVOption captcphook_options[] = {
    ...
    { "captcphook-test-fail-point",     "test fail point, in bytes",
        OFFSET(test_fail_point),        CAPAV_OPTION_INT(0, 0, INT_MAX) },
    { NULL }
};

static const AVOption caphttphook_options[] = {
    { "capinject-opaque",               "private data of user, passed with custom callback",
        OFFSET(opaque),                 CAPAV_OPTION_INT64(0, INT64_MIN, INT64_MAX) },
    { "capinject-segment-index",        "segment index of current url",
        OFFSET(segment_index),          CAPAV_OPTION_INT(0, 0, INT_MAX) },
    { "caphttphook-test-fail-point",    "test fail point, in bytes",
        OFFSET(test_fail_point),        CAPAV_OPTION_INT(0, 0, INT_MAX) },
    { NULL }
};
...
static const AVClass captcphook_context_class = {
    .class_name = "TcpHook",
    .item_name  = av_default_item_name,
    .option     = captcphook_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

URLProtocol capff_captcphook_protocol = {
    .name                = "captcphook",
    .url_open2           = captcphook_open,
    .url_read            = capurlhook_read,
    .url_write           = capurlhook_write,
    .url_close           = capurlhook_close,
    .priv_data_size      = sizeof(Context),
    .priv_data_class     = &captcphook_context_class,
};

static const AVClass caphttphook_context_class = {
    .class_name = "HttpHook",
    .item_name  = av_default_item_name,
    .option     = caphttphook_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

URLProtocol capff_caphttphook_protocol = {
    .name                = "caphttphook",
    .url_open2           = caphttphook_open,
    .url_read            = caphttphook_read,
    .url_write           = capurlhook_write,
    .url_seek            = caphttphook_seek,
    .url_close           = capurlhook_close,
    .priv_data_size      = sizeof(Context),
    .priv_data_class     = &caphttphook_context_class,
};
