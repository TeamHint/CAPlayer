...
#include "capavformat/capavformat.h"
...
#endif
    capmeta_set_avformat_context_l(ffp->meta, ic);
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0)
        capmeta_set_int64_l(ffp->meta, CAPM_KEY_VIDEO_STREAM, st_index[AVMEDIA_TYPE_VIDEO]);
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0)
        capmeta_set_int64_l(ffp->meta, CAPM_KEY_AUDIO_STREAM, st_index[AVMEDIA_TYPE_AUDIO]);
...
inline static int log_level_av_to_cap(int av_level)
{
    int cap_level = CAP_LOG_VERBOSE;
    if      (av_level <= AV_LOG_PANIC)      cap_level = CAP_LOG_FATAL;
    else if (av_level <= AV_LOG_FATAL)      cap_level = CAP_LOG_FATAL;
    else if (av_level <= AV_LOG_ERROR)      cap_level = CAP_LOG_ERROR;
    else if (av_level <= AV_LOG_WARNING)    cap_level = CAP_LOG_WARN;
    else if (av_level <= AV_LOG_INFO)       cap_level = CAP_LOG_INFO;
    // AV_LOG_VERBOSE means detailed info
    else if (av_level <= AV_LOG_VERBOSE)    cap_level = CAP_LOG_INFO;
    else if (av_level <= AV_LOG_DEBUG)      cap_level = CAP_LOG_DEBUG;
    else if (av_level <= AV_LOG_TRACE)      cap_level = CAP_LOG_VERBOSE;
    else                                    cap_level = CAP_LOG_VERBOSE;
    return cap_level;
}

inline static int log_level_cap_to_av(int cap_level)
{
    int av_level = CAP_LOG_VERBOSE;
    if      (cap_level >= CAP_LOG_SILENT)   av_level = AV_LOG_QUIET;
    else if (cap_level >= CAP_LOG_FATAL)    av_level = AV_LOG_FATAL;
    else if (cap_level >= CAP_LOG_ERROR)    av_level = AV_LOG_ERROR;
    else if (cap_level >= CAP_LOG_WARN)     av_level = AV_LOG_WARNING;
    else if (cap_level >= CAP_LOG_INFO)     av_level = AV_LOG_INFO;
    // AV_LOG_VERBOSE means detailed info
    else if (cap_level >= CAP_LOG_DEBUG)    av_level = AV_LOG_DEBUG;
    else if (cap_level >= CAP_LOG_VERBOSE)  av_level = AV_LOG_TRACE;
    else if (cap_level >= CAP_LOG_DEFAULT)  av_level = AV_LOG_TRACE;
    else if (cap_level >= CAP_LOG_UNKNOWN)  av_level = AV_LOG_TRACE;
...
static void ffp_log_callback_brief(void *ptr, int level, const char *fmt, va_list vl)
{
    ...
    int ffplv __unused = log_level_av_to_cap(level);
    VLOG(ffplv, CAP_LOG_TAG, fmt, vl);
}
...
{
    int av_level = log_level_cap_to_av(log_level);
    av_log_set_level(av_level);
}

void ffp_global_set_inject_callback(cap_inject_callback cb)
{
    capav_register_inject_callback(cb);
}

void ffp_io_stat_register(void (*cb)(const char *url, int type, int bytes))
{
    // avcap_io_stat_register(cb);
}
...
{
    // avcap_io_stat_complete_register(cb);
}
...
static void ffp_show_version_int(FFPlayer *ffp, const char *module, unsigned version)
{
    av_log(ffp, AV_LOG_INFO, "%-*s: %u.%u.%u\n",
           ...
           (unsigned int)CAPVERSION_GET_MAJOR(version),
           (unsigned int)CAPVERSION_GET_MINOR(version),
           (unsigned int)CAPVERSION_GET_MICRO(version));
}
...
int ffp_prepare_async_l(FFPlayer *ffp, const char *file_name)
{
    ...
    if (strlen(file_name) + 1 > 1024) {
        ...
        if (avio_find_protocol_name("caplongurl:")) {
            av_dict_set(&ffp->format_opts, "caplongurl-url", file_name, 0);
            file_name = "caplongurl:";
        }
    }
...
void ffp_check_buffering_l(FFPlayer *ffp)
{
    ...
    if (hwm_in_ms > 0) {
        ...
        if (video_cached_duration > 0 && audio_cached_duration > 0) {
            cached_duration_in_ms = (int)CAPMIN(video_cached_duration, audio_cached_duration);
...
CapMediaMeta *ffp_get_meta_l(FFPlayer *ffp)
{
    if (!ffp)
        return NULL;
